...
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get -y install chromium-browser && apt-get -y --purge autoremove && apt-get clean && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/*

#Run chromium in xpra
CMD ["run_in_xpra", "chromium-browser --no-sandbox"]
